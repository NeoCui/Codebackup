...
def Chk_RetCode (Commands):
    ...
    if ret <> 0:
        ...
        val = '8.15.3: fail Execution Commands error'
...
log_file='/mnt/8.15.3.log'
print (log_path)
shutil.copy (log_file,log_path)
fp1 = open('/mnt/8.15.3.log', "r")
Info=None
while 1:
    line = fp1.readline()
    if not line:
...
if Info is None:
    val = '8.15.3: fail No ADDR has been found'
    f5 = open('/mnt/report.txt','a')
    print >>f5,val
    Path=os.popen('ls /mnt/ | grep ^log_')
    log_path='/mnt/'+Path.read().split('\n')[0]+'/'
...
fp2 = os.popen("cat /mnt/8.15.3.log").read()
